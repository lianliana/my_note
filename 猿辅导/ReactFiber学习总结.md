...
> 基于阅读 `react-reconciler` 源码（commit：`1fb18e22ae66f...`，对应 React 16/17 时代实现）的讨论整理。
> React 18+ 在 Lanes/Flags 上有重构，但核心思路仍是同一套（演进对比另见 `React学习总结.md`）。
...
> 注意：本节描述的是 React 16/17 的实现，React 18 已演进为 `flags + subtreeFlags`（见 4.4），但解决的问题与总体思路一致。
...
#### 4.4 React 18 的演进：subtreeFlags

- 取消 `firstEffect / nextEffect` 单链表
- 每个 Fiber 自身的 `flags` 保留（含义类似旧 `effectTag`）
- 新增 `subtreeFlags`：聚合子树里所有 Fiber 的 flags 到父节点
- commit 阶段递归向下时，**靠 `subtreeFlags` 判断"这棵子树里有没有需要处理的副作用"**，没有就整棵跳过；有再递归
- 优点：和 Concurrent 模型更兼容（避免维护单链表带来的中断恢复复杂度），且可以做"子树级"的 bailout
...
- commit：根据 effectList（或 18 的 subtreeFlags）执行 Placement，把 `div`（以及其内部已经拼好的子树）插入容器
...
> 例外：React 19 的 `use(promise)` / `use(context)` 是目前**唯一**允许条件调用的 hook。

---

### 7. 你可以如何继续深入（可选路线）

- **React 16/17 路线**：从 `ReactFiberWorkLoop.new.js` 的 `performUnitOfWork -> completeUnitOfWork` 走一遍，观察 effectList 如何被拼接
...
- **React 18 路线**：阅读 `ReactFiberLane.js`（Lanes 模型）、`ReactFiberFlags.js`（subtreeFlags 的位定义与传播）、`ReactFiberWorkLoop.js` 的并发循环（`workLoopConcurrent` / `shouldYield`）
