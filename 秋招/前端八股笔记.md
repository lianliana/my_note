...
块级元素：div p ul table form ol  audio video Footer li h1 h2 h3  header

行内元素：a span img strong  button input select label picture
...
[掘金解决1px问题各种方案](https://juejin.cn/post/6844903877947424782#heading-4)
...
[掘金好文](https://juejin.cn/post/6995006409423192078#heading-1)
...
### flex:1 flex:auto flex:0 flex:none区别

[掘金好文](https://juejin.cn/post/6971010647492395044)

flex 包含flex-grow flex-shrink flex-basis
...
2. Object.getOwnPropertyNames(obj) 回一个数组，包含对象自身的所有属性（不含 Symbol 属性，但是包括**不可枚举属性**）的键名
...
### Object.defineProperty、Proxy、Reflect区别和用法

[defineProperty掘金好文](https://juejin.cn/post/6995071499098914824)

[Proxy和Reflect掘金好文](https://juejin.cn/post/6844903790739456013#heading-0)



### ES6-ES12新语法

[掘金好文讲解ES6+](https://juejin.cn/post/6995334897065787422#heading-43)

ES6：

- ...展开运算符
- 默认参数 function fn(a = 3){}
- promise
- 箭头函数
- const、let
- Array中filter、map、forEach
- 模板字符串
- 解构赋值
- 对象属性同名省略
- 剩余参数
...
- ?.替代 obj && obj.name
- ?? 替代 || 区别是 在？？中只有undefined和null才是false
- BigInt 、Symbol
- fromEntries 可以把键值对转换成对象
- Promise.allSettled
- Promise.any
- 字符分隔符
...
- 因为线程是通过importScripts引入外部的js，并且直接执行，其实是不安全的，很容易被外部注入一些恶意代码
...
var a1 = new A()
A.prototype.x = 10
//到这行之前都是A.prototype的指针都是指向a1.__proto__上的 ，并且a1.__proto__上的对象已经生成 详情可看下一题
A.prototype = {x:20,y:30} //这一行改变了A.prototype的指针指向 所以后面a2不一样了
var a2 = new A()
...
[知乎讲解sort源码](https://zhuanlan.zhihu.com/p/33626637)

数据量小于10的时候使用插入排序

数据量大于10使用快排+插入排

这里快排有个优化方法就是选取关键值，就是通过步长 = 200 + 数组长度&15
